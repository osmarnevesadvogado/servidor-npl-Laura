...
async function processBufferedMessage(phone, text, senderName, respondComAudio = false, instancia = null) {
  try {
    ...
    // generica e deixar a Laura responder direto com atendimento premium (prompt cuida disso).
    if (ehPrimeiroContato) {
      primeiroContatoEnviado.add(cleanPhonePrimeiroContato);

      const ehClienteReconhecido = contexto && contexto.tipo === 'cliente';

      if (ehClienteReconhecido) {
        // Cliente reconhecido pelo telefone — Laura responde direto com modo premium.
        // Nao enviamos a apresentacao generica pra ele nao se sentir desrespeitado
        // ("a Laura nao me reconhece"). Deixamos o fluxo normal seguir abaixo.
        await db.trackEvent(conversa.id, lead?.id, 'primeiro_contato', `${senderName} (cliente CRM)`);
